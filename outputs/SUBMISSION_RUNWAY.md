...
- **Visual Help & Tips**: annotated screenshots (arrows/circles showing where to tap) in Settings → Help & Tips. Cheap to build with existing tooling (simctl pipeline + compose-screenshots.py Pillow overlays). Partly redundant with the live coach-marks → lower priority.
  - **SUPERSEDED 2026-09-12 by `outputs/PLAN_TUTORIAL_AND_HELP.md`.** "Lower priority" was a ranking with no case under it. There is now a case: the first written contact from a real App Store user asked for two things the app already answers well, and his usage summary says he had reached none of them. The plan carries that evidence and a pre-registered acceptance test. The "partly redundant with the live coach-marks" reasoning is **not** retired — it is one of three arguments the plan leaves open, and one of the three is a NotebookLM citation that has never been re-asked.
