...
| **D51** | **FIELD REPORT — Analytics: everything above the "Аналитика" title stopped responding, once** (TestFlight, Elena Logacheva, iPhone 13, iOS 26.6.1, Europe/Moscow, 2026-09-24 12:00, **1.0.6 build 11**). Verbatim RU in `STATE.md` §9.4. Observable: controls ABOVE the nav title unresponsive, everything BELOW worked; not a crash; cleared after the app was closed ~20 min; re-tried 3× that day, no recurrence. Screenshot: Horizon ("Тренды"), Combined, April 2026 selected, +0,00 ₽ — i.e. the sticky month tooltip was up. **The only controls above the title are the navigation bar's: in 1.0.6 that is the Reports entry point (`AnalyticsView.swift:136–146`)** | user-experienced: a dead nav bar until relaunch; not data-affecting | **YES, once, on a real device, Release build.** Rate unknown (n=1 of 4 attempts) | 1.0.6 b11 (TestFlight). Whether earlier versions carried it is unknown — the toolbar button is new in 1.0.6, but the Horizon month sheet (`AnalyticsHorizonView.swift:69–72`) shipped in 1.0.3 | **Probe, not a guard:** `AnalyticsToolbarAfterSheetProbeTests` (4 variants: Reports sheet dismissed by swipe / by Done / backgrounded mid-dismiss; Horizon month sheet dismissed by swipe; 8 cycles each; on a miss, records `isHittable`, window count, and whether a scroll revives the button) | **OPEN — NOT REPRODUCED, attempt 1 (2026-09-24): 5 variants × 8 cycles, iOS 26.5 simulator, Debug, all presented — see `STATE.md` §9.4 for what was and was NOT exercised (no data-reload-under-sheet, no device / 26.6.1 / Release, no real interruption).** Prime suspect: a dismissed sheet leaving the navigation bar unable to receive touches — the documented SwiftUI class (Apple forums 131404, 685163, 692338: toolbar dead after a swipe-dismissed sheet; a scroll revives it). Two presenters of the same `ReportsScreen` exist: `AnalyticsView.swift:147` (toolbar) and `ContentView.swift:187` (notification / Siri) — noted, not implicated. Three follow-ups pending from Elena (opened a report just before? interruption? did scrolling revive it?) — **do not conclude before they arrive** | this row; `STATE.md` §9.4 | HEAD 2026-09-24 |
...
| **D33** | `wipeLedger` is a **45 764 ms main-thread block** and the ATTRIBUTED cause of the three recurring UI-suite reds (`STATE.md` §9.2) | *"It is not a race that sometimes loses — it is a race that cannot be won"* (`:98`) | **No** — DEBUG-only; *"`--seed-large-dataset` is **absent from the Release binary's symbols**. No user can reach this path."* (`:240–242`) | DEBUG builds only | **YES** — `PurgeCostMeasurementTests.test_wipeLedgerCost_at8kRows` | **OPEN — PRIORITY RAISED 2026-09-24, and its previous classification MAY HAVE BEEN WRONG.** For a month the three intermittent tests were explained by this seam and read as ignorable. D51 is an independent sighting on a **real device in a Release build**, where this seam does not exist — so *if* D51 and the tests share a cause, that cause is not `wipeLedger`, and the "attributed" above is a misattribution that hid a shipped defect. **Honest caveat:** the tests' recorded failure messages are *"list never showed rows"*, *"demo seed missing"*, *"Save button missing"* — elements/data ABSENT — while D51 is a visible control that ignores a tap. Same family (a tap yields nothing), not the same observable. **Confirms a shared cause:** unresponsive hit-testing reproduced in a Release build (or any build without the seam). **Refutes it:** the Analytics symptom reproduced and traced to something the tests never reach (e.g. the sheet-dismiss class), while the tests still fail only with the seam. Remedy stays first after 1.0.6 ships; the *investigation* is no longer deferred (`STATE.md` §9.4) | `BRIEF_UI_SHARED_CONTAINER_RESIDUE_2026-08-14.md:89–98`, `:222–224`; D51 | HEAD 2026-09-24 |
