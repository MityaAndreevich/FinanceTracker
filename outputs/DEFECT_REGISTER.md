...
**Derived 2026-09-12 at HEAD `537e220`.** Status lives in `outputs/STATE.md`; this file is its defect
half and obeys the same rules — **every row cites a file and a line, and no row was written from
memory.**
...
current** · 1.0.5 b9/b10 = *in review, not shipped*.
...
| **D2** | CSV import accepts amounts the UI refuses → `Int` overflow traps on the dashboard's first render | *"bricks the app"* (`:91`); *"Silent corruption, in those words"* (`:120`) | **YES**, via a hand-made or malformed CSV. *"Not claimed that any user has hit this."* (`:234`) | **1.0.0 → 1.0.4 b8.** Fixed by `1b6be14`, `c2461b3` — **build 10 only** | **YES** — `PoisonedAmountLaunchTests` walks cold launch → dashboard unavailable state → Transactions → delete → totals return (`:165–176`); `AmountParsingTests.testLeadingDecimal` caught the fix's own regression (`:192`) | **FIXED in build 10** (`:129`) — **but its own header at `:3` still says *"open… nothing touched"***. See `STATE.md` §4.2 | `DEFECT_IMPORT_AMOUNT_CAP_ASYMMETRY.md` | HEAD 2026-09-12 (tags) |
| **D3** | 1.0.0-era stores stranded on the migration floor (declared V1 carries `isPossibleDuplicate`, added the day after 1.0.0 shipped) | *"Field bug"* (`:1`); terminal — the user's escape was *"deleting and reinstalling, which discards the store"* (`:6`) | **YES — live in the field.** Anyone whose most recent **launch** was 1.0.0, updating to 1.0.3+ (`:85`). 1.0.3 users unaffected, *"confirmed, not assumed"* (`:91`) | **Live since 1.0.3 became available 2026-07-29** (`:98`) — through **1.0.4 b8**. Fixed by `8c748b7`, **build 9** | **YES, and proven to fail without the fix** — `PreV1StoreMigrationTests`, 4 tests; with the lift commented out, 2 of 4 fail (`:227–230`) | **FIXED in 1.0.5, NOT RELEASED.** Header at `:3` says *"Not yet committed"* — **half stale**, see `STATE.md` §4.3 | `BUG_MIGRATION_FLOOR_1_0_0_STORES_2026-08-14.md` | HEAD 2026-09-12 (tags) |
...
| id | title | severity | user-reachable? | present in which shipped versions | regression test? | status | document | verified |
|---|---|---|---|---|---|---|---|---|
| **D5** | **14 `Int`-accumulation expressions still trap on an unrepresentable amount** — `AnalyticsSeries` (6), `CategoryDetailView` (2), `DaySpendingSheet` (2), `AnalyticsView` (1), `AnalyticsBreakdownView` (1), `EditTransactionView` (1), `CSVImportService` (1) | *"`AnalyticsSeries` is the one that matters"* (`GO_LIVE_CHECKLIST.md:71`) | **YES.** *"Analytics still traps on a ledger the dashboard has just told the user is otherwise fine."* (`DEFECT_IMPORT_AMOUNT_CAP_ASYMMETRY.md:202`) | **every shipped version, and HEAD** | **NO** — the journey test deliberately stops at the recovery path: *"None is on the journey from cold launch to deleting the row, which is why they are not in build 10."* (`:226`) | **OPEN** | `DEFECT_IMPORT_AMOUNT_CAP_ASYMMETRY.md:221–230` · `GO_LIVE_CHECKLIST.md:66–77` | **HEAD 2026-09-12 — re-counted today.** `grep -c addingReportingOverflow` returns **0** in `AnalyticsSeries.swift`, `Analytics/CategoryDetailView.swift`, `Analytics/DaySpendingSheet.swift`, `AnalyticsView.swift`, `Analytics/AnalyticsBreakdownView.swift`. `AnalyticsSeries.swift:92,94` still reads `.income += tx.amountCents` / `.expense += tx.amountCents` |
...
### 3.1 Reachable in the currently shipped version (1.0.4 build 8)

| id | title | severity | user-reachable? | present in which shipped versions | regression test? | status | document | verified |
|---|---|---|---|---|---|---|---|---|
| **D7** | **`idTBD` in the App Store URL — "Rate the app" and "Share" land on an App Store error page** | not stated; *"One-line fix"* (`:51`) | **YES** — *"a live 'Rate the app' / 'Share' link that lands on an App Store error page"* (`:48`) | **1.0.0 → 1.0.4 b8.** Fixed in the tree | **No** — *"no test needed beyond opening the link"* (`:51`) | **FIXED in the tree, NOT RELEASED** | `AUDIT_BACKLOG_VERIFIED_2026-08-12.md:48` | **HEAD 2026-09-12.** `v1.0.4-build8:…/AboutView.swift:57` = `…/idTBD`; HEAD `:59` = `…/id6784424678` |
| **D8** | **"Restart onboarding" in Settings is a silent no-op behind a destructive confirmation alert** | not stated | **YES** — *"the user taps a destructive-styled confirmation alert… and **nothing happens**"* (`:312`) | **through 1.0.4 b8.** Removed in the tree per `PROPOSAL_1_0_5_SCOPE.md:103` | **No** — and its sibling `requestReplay()` had *"a **passing** test that exercised a path the app never takes"* (`AUDIT_TEST_REACHABILITY_2026-08-13.md:5`) | **REMOVED in the tree, NOT RELEASED** | `AUDIT_BACKLOG_VERIFIED_2026-08-12.md:299`, `:312–314` | **HEAD 2026-09-12.** `restartOnboarding` present in `v1.0.4-build8:…/GeneralSettingView.swift:154`; **zero** grep hits in `FinanceTracker/` at HEAD |
| **D9** | **Month-end recurring series drifts off month-end permanently** (Jan 31 → Feb 28 → Mar 28 → …) | not stated | **YES** — any monthly series anchored after the 28th | **1.0.0 → 1.0.4 b8** | Not established here | **FIXED in the tree, NOT RELEASED** | `AUDIT_BACKLOG_VERIFIED_2026-08-12.md:54–62` · `BRIEF_MONTHEND_RECURRENCE_DRIFT.md:24` | **HEAD 2026-09-12.** `nextDate(after:anchor:)` with `min(anchorDay, daysInMonth(of: last))` exists at `RecurrenceType.swift:119–128`; **absent from `v1.0.4-build8`** (`grep -c "anchor: Date"` = 0) |
| **D10** | **PDF export clips amounts silently** (fixed-width amount column, no paragraph style) | not stated | **YES** — reported by a real user | **1.0.3, 1.0.4 b8** (three shipped versions with zero tests) | **YES, now** — `f7dde93`, `891bcb1` render real PDFs and read pixels back | **FIXED in build 10, NOT RELEASED** | `git log f7dde93`, `891bcb1`, `c8bea4f` | HEAD 2026-09-12 (tags) |
| **D11** | **`ReverseTrial.isActive` unclamped — a rewound device clock restores premium indefinitely** | *"unbounded, not one fortnight"* (`OVERNIGHT_2026-08-05.md:292`) | YES, but self-punishing — *"it breaks every other app and iOS re-syncs time"* (`OVERNIGHT:294`) | present through 1.0.4 b8 | **Had one, and it never discriminated** — *"`clockMovedBackwardsStaysActive` passes with and without a clock clamp… It never discriminated."* (`AUDIT_TEST_DISCRIMINATION_2026-08-08.md:8`) | **FIXED in the tree** (persisted monotonic watermark), **NOT RELEASED** | `AUDIT_BACKLOG_VERIFIED_2026-08-12.md:132–135` · fix scoped `PROPOSAL_1_0_5_SCOPE.md:494` | **HEAD 2026-09-12.** `ReverseTrial.swift` now documents *"never earlier than the latest date it has ever seen"* |
...
| **D41** | "Import failed" shown after a partial commit | `AUDIT_CSV_IMPORT_DUPLICATION_2026-08-13.md:210–220`, severity *"**HIGH**"* (`:237`) | **FIXED in 1.0.5** — *"the disclosure half is BUILT (1.0.5)"* (`:5`). **NOT RELEASED** | doc |
